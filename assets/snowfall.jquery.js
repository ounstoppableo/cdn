...
(function ($) {
  $.snowfall = function (element, options) {
    ...
    function Flake(_x, _y, _size, _speed, _noRotate) {
      ...
      this.update = function () {
        ...
        if (this.rotateIncreaseFlag) {
          ...
          if(this.rotate>=60 || this.rotateXAngle<=0 || this.rotateYAngle >= Math.PI/2){
          ...
          if(this.rotate<=0 || this.rotateXAngle>=Math.PI /2 || this.rotateYAngle <= 0){
            this.rotateIncreaseFlag = true;
          }
        }

        const rotate = this.noRotate ? '' : `rotate3d(${rotateXAxle}, ${rotateYAxle}, 0, ${this.rotate}deg)`;
    ...
    for (i = 0; i < options.flakeCount; i += 1) {
      flakes.push(new Flake(random(widthOffset, elWidth - widthOffset), random(0, elHeight), random((options.minSize * 100), (options.maxSize * 100)) / 100, random(options.minSpeed, options.maxSpeed), options.noRotate));
    }
